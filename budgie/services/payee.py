...
async def get_payees(
    ...
    """
    result = await db.execute(select(Payee).where(Payee.user_id == user_id))
    payees = list(result.scalars().all())
    for payee in payees:
        decrypted_name = decrypt_str(payee.name, session_key) or payee.name
        set_committed_value(payee, "name", decrypted_name)
...
async def create_payee(
    ...
    # Return with plaintext name for the API response (without marking ORM dirty)
    set_committed_value(payee, "name", schema.name)
    return payee
